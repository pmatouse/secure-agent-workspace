...
def _check_k8s_status(spec, meta, namespace):
    ...
    # Check gateway pod (gateway container, not all sidecars)
    gateway_ready = False
    try:
        pods = v1.list_namespaced_pod(
            session_ns,
            label_selector=f"app.kubernetes.io/name={session_name},app.kubernetes.io/component=gateway",
        )
        for pod in pods.items:
            for cs in (pod.status.container_statuses or []):
                if cs.name == "openshell-gateway" and cs.ready:
                    gateway_ready = True
                    break
    except client.ApiException:
        pass
            ...
            for cond in conditions:
                if cond.get("type") == "Ready":
                    if cond.get("status") == "True":
                        sandbox_ready = True
                    elif cond.get("reason") in ("PodFailed", "PodSucceeded", "Error"):
                        sandbox_error = True
    except client.ApiException:
        pass
    ...
    if sandbox_error:
        _set_status(cr_name, namespace, "Error",
                    "Sandbox terminated", backend="kubernetes", sess_ns=session_ns)
        return
